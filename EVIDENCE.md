...
- The scoped bootstrap permission update from commit `4d8d54ebb2edfb30482d0c421f9e79ca92dce326` reached `UPDATE_COMPLETE`, the failed empty application stack was deleted, and CodeBuild run `axcas-beta-deploy:0a61558e-c95b-4934-837c-1bd4beb4af60` passed the prior security-group failure. CloudFormation created the security group, encrypted buckets, table and supporting resources, then rolled back because Cognito rejected a `phone_number` update-verification requirement that was absent from `AutoVerifiedAttributes`. A RED regression test reproduced the invalid pair; source now configures `phone_number` in both settings. TypeScript, 3 legacy tests, 333 Vitest tests, 12 Hermes-plugin tests, 8 AWS-media tests (356 total), both templates through `cfn-lint`, the production audit and whitespace gate passed. Another AWS deployment is not claimed.
- Clean retry `axcas-beta-deploy:1ca90b5e-2729-48c2-bfe8-ad2e1af578ef` built the exact `790d4c93d9e495dda9158627de6a2deb1e0ab598` images and passed the prior Cognito consistency error, but the application stack rolled back when Cognito required an SMS configuration for `AutoVerifiedAttributes: [phone_number]`. Axcas delivers its custom authentication challenge through WhatsApp and marks the number verified only from the Meta-signed admin provisioning path, so Cognito SMS auto-verification and update-verification settings have now been removed. A focused regression failed before that change and passed afterward. No endpoint or customer data resulted from this attempt; a full gate, new commit, failed-stack cleanup and retry are not yet claimed.
- Exact commit `cd0b71a0dc5270bc8d8cda1c55969d1c1d8f1eab` passed the 356-test local gate and GitHub production gates `35362675982` and `35362680418`. Confirmed retry `axcas-beta-deploy:a262e744-0a4b-452a-b34d-9bae4b4d750b` then passed the prior Cognito failures, but `PublicApiStage` failed because the scoped CloudFormation role lacked the granular `apigateway:TagResource` permission requested by the live API Gateway V2 resource provider. No endpoint or customer action resulted. Source now grants `TagResource` and `UntagResource` only on this stack's generated HTTP API stage ARN pattern; the resource-local `cfn-lint` suppression documents that version 1.57.0 still uses the older HTTP-verb-only API Gateway action catalog. A RED regression failed before the change and the focused tests and both template lints passed afterward. The IAM update, cleanup and another deployment are not claimed.
- Exact commit `cca0f3a179a7bb9cf3fcc42b0e9cdab3feabee5e` passed the 357-test local gate and GitHub production gates `35364334472` and `35364344501`. The confirmed bootstrap update reached `UPDATE_COMPLETE`, the empty failed application stack was deleted, and CodeBuild run `axcas-beta-deploy:e5861a6e-b727-49f9-900d-111664207190` built the exact images and started a fresh application stack. `PublicApiStage` failed again because the live API Gateway V2 provider authorizes stage tagging against the collection ARN ending in `/apis/<api-id>/stages`, not the child-stage pattern ending in `/stages/*`; dependent resources were cancelled and no endpoint or customer action resulted. A RED regression now preserves that exact observed ARN shape, and source narrows the same two granular actions to `arn:${AWS::Partition}:apigateway:${AWS::Region}::/apis/*/stages`. The corrected IAM update, failed-stack cleanup and another retry are not claimed.
- Exact commit `7288ba466b1c5318c7d9e3dfb18abba70a57c844` passed 361 local tests and both GitHub production gates. The confirmed bootstrap update and empty-stack cleanup succeeded, and CodeBuild run `axcas-beta-deploy:31fda9f6-430d-485d-9ab1-0569fdf1e0a1` built and pushed its immutable images. CloudFormation then rejected `ApiWebAclAssociation`: AWS WAF accepts an API Gateway REST API stage ARN but not the V2 HTTP API ARN `arn:aws:apigateway:ap-south-1::/apis/fbrrgyffug/stages/$default`. The stack rolled back and produced no endpoint. A RED regression now removes that invalid claim, preserves HTTP API stage throttling plus signature/JWT verification, and removes the unused WAF permissions; edge WAF is deferred to a supported CloudFront/custom-domain design. Cleanup and redeployment of this correction are not claimed.
- Exact commit `ed799fc4ce416cf907a35e673850428d90005410` passed 361 local tests and GitHub production gates `35368378103` and `35368384886`. The confirmed empty-stack deletion and corrected CodeBuild run `axcas-beta-deploy:944f0bef-401c-4301-aa53-3fba6cbd47fc` passed the former WAF failure and created the API, Cognito, queues, tables, buckets, Lambda functions and CloudFront distribution. The always-on `WorkerService` then triggered the ECS deployment circuit breaker; create rollback deleted the cluster, service and create-time worker logs, so no stable AWS endpoint or worker-runtime acceptance is claimed. The template now defaults `EnableWorkers=false` and conditions only the always-on service. Provider secrets remain placeholders; worker activation requires a standalone task acceptance receipt followed by an explicit `EnableWorkers=true` stack update. This preserves a deployable core/Studio acceptance path without misrepresenting WhatsApp as live.
- Exact commit `3c11aa5a8876c6721fe77daf53d68153f6f315b7` passed the complete local gate: TypeScript, both CloudFormation templates through `cfn-lint`, 3 legacy tests, 339 Vitest tests, 12 Hermes-plugin tests, and 8 AWS-media tests (362 total). The same commit passed GitHub production gates `35371393517` on `codex/axcas-production` and `35371397707` on `main`. After the user confirmed deletion of only the empty rolled-back `axcas-beta-native` stack, CodeBuild run `axcas-beta-deploy:2df33970-380e-4eb4-82f8-faf404cc7b27` completed `SUCCEEDED` at `2026-09-18T17:10:06.291000+00:00`, and the fresh `axcas-beta-native` application stack reached `CREATE_COMPLETE` in `ap-south-1`. Stack outputs expose public API `https://zu367dlc0k.execute-api.ap-south-1.amazonaws.com`, sites distribution `https://ddaq0xpqvmht2.cloudfront.net`, Amplify branch URL `https://beta.d297qepktfln3w.amplifyapp.com`, Cognito user pool `ap-south-1_YfcAtTYw0`, and Studio client `6sburbsgs2kgee31brcaf62ohi`. This receipt proves the AWS-native core stack only. `EnableWorkers=false`, the ECS worker service is intentionally absent, and provider secrets remain placeholders. The Studio artifact upload/job and browser rendering have not yet been verified, so the Amplify URL is not yet claimed as a usable Studio. No Meta/WhatsApp provider acceptance, public merchant onboarding, real merchant website, real-media reel, campaign publication, metrics loop, or end-to-end customer flow is claimed by this deployment.
- On 2026-09-19, an external HTTP smoke against that stack returned `401` for the two JWT-protected probes (`GET /api/studio/me` and `GET /oauth/meta/start`), but exposed three launch blockers: `GET /health` returned API Gateway `500`, unsigned `GET|POST /whatsapp/webhook` also returned `500`, the Amplify branch returned its generic welcome page, and the empty sites distribution returned `403`. Source tracing proved that the Lambda eagerly loaded provider secrets before routing even `/health`. A RED regression reproduced the missing-secret failure, and the GREEN change makes only `GET /health` independent of optional provider configuration; the complete local gate then passed TypeScript, 3 legacy tests, 340 Vitest tests, 12 Hermes-plugin tests, 8 AWS-media tests, and both CloudFormation templates through `cfn-lint` (363 tests total). The correction is not claimed live until its commit is built and deployed. A five-file Studio artifact was built locally for the exact AWS outputs, but its Amplify upload did not start because the AWS console session expired before `create-deployment`; the generic Amplify page remains the truthful live state.
- After AWS access was restored, Amplify manual deployment job `1` for app `d297qepktfln3w`, branch `beta`, completed `SUCCEED` from `2026-09-19T07:52:51.376000+00:00` to `2026-09-19T07:52:54.252000+00:00`. External HTTP returned `200`, the artifact title, API origin and production WhatsApp number were present, and a clean-browser inspection reached the passwordless WhatsApp-number form without submitting a number or claiming OTP delivery. Manual inspection also found the pre-submit status incorrectly said `Waiting for the message…`; a RED browser regression reproduced it and the GREEN source now says `Enter your number to receive the code.` The complete 363-test/type/CloudFormation gate passed again, but that wording correction is not claimed live until a new exact artifact deployment succeeds. CodeBuild run `axcas-beta-deploy:d02e0659-eaa3-4ef1-939a-4bc8b2dceb55` for commit `dc00d39b029c92c4fd460a47faeec200600e566d` is in progress; no backend correction is claimed live from a running build.
- CodeBuild run `axcas-beta-deploy:d02e0659-eaa3-4ef1-939a-4bc8b2dceb55` subsequently completed `SUCCEEDED` at `2026-09-19T07:57:34.729000+00:00`, but the repeated external probe still returned `500` for `/health` and the public WhatsApp routes. CloudWatch then exposed the earlier hidden initialization fault: `Dynamic require of "node:https" is not supported` from the esbuild ESM bundle while loading the AWS SDK. A RED architecture regression now requires the Lambda-only control-plane image to use CommonJS, the Dockerfile emits and copies `.js` CommonJS bundles, and an independent local bundle/import probe returned `CONTROL_PLANE_CJS_IMPORT_OK`. This packaging correction is not claimed live until its own exact gated commit is built, deployed, and the public probes pass.
- Exact packaging commit `1046f45ac3009d9052268ec9ecaee7f3bdb1b5ad` passed the complete local gate and GitHub production gates `35430962497` and `35430960454`. CodeBuild run `axcas-beta-deploy:d2e3e5bd-9b96-4e1b-badc-3c477f062d2d` completed `SUCCEEDED` at `2026-09-19T08:10:33.213000+00:00`, and the `axcas-beta-native` stack reached `UPDATE_COMPLETE` at `2026-09-19T08:09:31.817000+00:00`. The repeated external probes then returned the expected safe boundary results: `GET /health` `200 {"status":"ok"}`, unauthenticated Studio and Meta OAuth routes `401`, invalid WhatsApp verification `403`, unsigned WhatsApp `POST` `401`, and Studio `200`. Amplify job `2` completed `SUCCEED` from `2026-09-19T08:06:02.951000+00:00` to `2026-09-19T08:06:11.780000+00:00`; a clean-browser retest confirmed the corrected `Enter your number to receive the code.` state. These receipts prove a live AWS core/API boundary and customer-facing Studio shell, not provider acceptance: `EnableWorkers=false`, provider secrets are placeholders, the sites distribution has no merchant artifact, and no OTP, signed WhatsApp intake, website release, real-media reel, campaign publication, insights loop, or two-tenant journey has passed.

- AWS worker hardening on 2026-09-19: commit `954c9c376bc16092c6b9200aea75e6e013c54b22` deployed through CodeBuild run `axcas-beta-deploy:2b00e943-ec56-4c79-ba24-85818fbea4f7`, which completed `SUCCEEDED` at `2026-09-19T08:37:35.237000+00:00`; the stack returned `UPDATE_COMPLETE`, `/health` returned `200 {"status":"ok"}`, and Studio returned `200`. The source adds native Hermes Bedrock configuration using the ECS task role, streaming/discovery IAM permissions, fixed non-root UIDs, a private shared GID, and a one-shot `0770` runtime-volume initializer. An attempted retained-secret schema cleanup caused `ProviderSecrets` update and rollback to fail because the scoped CloudFormation role intentionally cannot read the secret; the resource had not mutated, rollback was completed by skipping only that resource, and the template now preserves its legacy placeholder shape while not injecting OpenRouter into Hermes. Provider preflight remains truthful: `PROOFGATE_SERVICE_SECRET` is configured, but all six Meta production values are still placeholders and `EnableWorkers=false`.
- Isolated ECS acceptance then found a real worker packaging failure: task `4b6aebd9b4ca49c7b626e49a28654bf9` exited `tool-bridge` with `Dynamic require of "child_process" is not supported`; it consumed no queue because relay was overridden with `sleep`, and it was stopped. A direct CommonJS correction was rejected before deployment because esbuild cannot emit the relay's top-level `await` as CJS. The final source keeps ESM, externalizes third-party runtime packages, prunes dev dependencies, and copies production `node_modules`; local bundle output, TypeScript, both CloudFormation lints, 3 legacy tests, 342 Vitest tests, 12 Hermes plugin tests, and 8 AWS media tests passed (365 total), followed by GitHub production gates on both branches.
- Exact worker fix commit `cf9a4736ca1a1812ae9307147f6ecf11825926ba` deployed through CodeBuild run `axcas-beta-deploy:07c2c178-e98e-42ba-ac2a-15cda471df71`, which completed `SUCCEEDED` at `2026-09-19T08:52:35.228000+00:00`; the stack returned `UPDATE_COMPLETE`. Isolated boot task `b158a86ca16b4ffd9a13b17018ccbc2f` remained `RUNNING`: runtime-init exited `0`, Hermes, inert relay, and tool-bridge were all running, and the bridge logged `{"service":"axcas-tool-bridge","status":"listening"}` before the task was deliberately stopped. A second isolated task `e8f7a47458ed4b2084d0b4d9b1876cda` invoked `global.anthropic.claude-haiku-4-5-20251001-v1:0` through the ECS task role and logged exact receipt `AXCAS_BEDROCK_OK`, then was deliberately stopped. These prove image boot, private socket setup, bridge readiness, and Bedrock task-role inference only. They do not prove Meta authentication, signed WhatsApp intake, OTP/template delivery, an actual merchant site, merchant-media reel, publication, metrics, or learning loop; workers remain disabled until real Meta credentials pass provider acceptance.
...
- **Observed model failure:** after relay delivery, Bedrock returned HTTP 404 because the account had not submitted Anthropic first-time-use details. The earlier one-shot task-role receipt did not establish durable Anthropic entitlement. Axcas therefore moved the customer runtime to Amazon Nova 2 Lite through `global.amazon.nova-2-lite-v1:0`, an AWS-owned model visible in this account's Mumbai Bedrock catalog; live inference still requires a fresh receipt after deployment.
